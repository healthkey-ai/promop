...
@pytest.mark.parametrize('note_id', [1, 10**15, 2**63 - 1])
def test_genomic_overflow_reference_fits_cdm_width_with_large_note_ids(monkeypatch, note_id):
    from omop_core.services.clinical_text import OwnedTextReader
    from omop_core.services.genomics import _read_note_text, _store_text

    row = MeasurementFactory()
    monkeypatch.setattr('omop_core.services.clinical_text.next_pk', lambda model, field: note_id)
    value = 'Original laboratory narrative. ' * 20
    row.value_as_string = _store_text(value, row, row.pk)
    assert len(row.value_as_string) <= 60
    assert row.value_as_string == f'[note:{note_id}]'
    assert _read_note_text(row, row.pk, OwnedTextReader(row.person_id, [row])) == value
