...
The implemented baseline includes [#1249](https://github.com/healthkey-ai/promop/pull/1249), [#1259](https://github.com/healthkey-ai/promop/pull/1259), [schema audit #1263](https://github.com/healthkey-ai/promop/pull/1263), [PALB2 naming #1281](https://github.com/healthkey-ai/promop/pull/1281), [project compatibility #1294](https://github.com/healthkey-ai/promop/pull/1294), [TP53 ternary fix #1360](https://github.com/healthkey-ai/promop/pull/1360), [concept display names #1355](https://github.com/healthkey-ai/promop/pull/1355), and [CancerBot field mapping reconciliation #1284](https://github.com/healthkey-ai/promop/pull/1284). Updated 2026-09-16 with all 1.3 RC gates passed and TP53 ternary semantics implemented. The [implemented architecture](genomics_architecture.md) describes runtime behavior; this plan records delivery status, remaining requirements and acceptance criteria. The supplied Word documents are retained as requirements through these two canonical documents.
...
**All three 1.3 RC gates passed on 2026-09-15.** The staging release candidate is accepted. Live production deployment remains a separate release step gated by #1286.

| Issue | Blocks 1.3 RC? | Status |
| --- | --- | --- |
| [#1315](https://github.com/healthkey-ai/promop/issues/1315) — TP53 compatibility and cache reconciliation | **Passed** | EXACT #483 merged. PRomop #1321 merged with staging reconciliation receipts. TP53 ternary semantics (#1360) implemented: true/false/null contract verified by smoke test. |
| [#1316](https://github.com/healthkey-ai/promop/issues/1316) — upgrade and writer readiness | **Passed** | All migrations applied to isolated production schema copy. Combined preflight audit passed (`ready: true`, 17/17 prerequisites). Actor concept 32817 restoration path demonstrated. |
| [#1317](https://github.com/healthkey-ai/promop/issues/1317) — final staging smoke | **Passed** | All seven smoke API checks passed on Render staging (catalog, create_and_positive_readback, absent_readback, indeterminate_readback, present_readback, source_provenance, delete_and_unknown_readback). Ternary TP53 readback verified: absent→false, indeterminate→null, present→true. |
| [#1286](https://github.com/healthkey-ai/promop/issues/1286) — live production prerequisites | **Blocks production release** | Production still needs its migration chain, authoritative actor concept 32817 restoration through vocabulary maintenance, and a passing combined audit on the deployed revision. Preserve curator decisions and handle vocabulary publication metadata consistently. Do not close this issue based on staging or rehearsal evidence. |
...
| TP53 ternary semantics (true/false/null) | [#1359](https://github.com/healthkey-ai/promop/issues/1359) | Closed; merged in #1360. `tp53_disruption` returns true (pathogenic present), false (tested negative/absent), null (unknown/indeterminate). Smoke-verified on staging. |
| TP53 disease matching fix | [#1357](https://github.com/healthkey-ai/promop/issues/1357) | Closed; merged. Disease prevalence matching uses substring instead of exact name match. |
| Concept display names on History tab | [#1355](https://github.com/healthkey-ai/promop/issues/1355) | Closed; merged. OMOP clinical endpoints expose `concept_name` with `select_related` optimization. |
| CancerBot field mapping reconciliation | [#1284](https://github.com/healthkey-ai/promop/issues/1284) | Closed; merged. Source inventory reconciled and priority field mappings audited. |
| TP53 cache reconciliation | [#1321](https://github.com/healthkey-ai/promop/pull/1321) | Closed; merged. Scoped preview/apply of ternary TP53 rule from source findings. |
| TP53 sample data seeding | [#1352](https://github.com/healthkey-ai/promop/pull/1352) | Closed; merged. Plausible tp53_disruption values seeded for sample patients. |
| Effective finding state and shared dialog | [#1240](https://github.com/healthkey-ai/promop/issues/1240) | Core implementation merged in #1249; true/unknown TP53 correction merged in #1297; ternary semantics merged in #1360; clinical applicability, negative/combined aggregation and consumer review remain |
...
- Define evidence-aware TP53/del(17p) aggregation. [PR #1297](https://github.com/healthkey-ai/promop/pull/1297) merged the initial true/unknown correction; [PR #1360](https://github.com/healthkey-ai/promop/pull/1360) completed the ternary contract: true (pathogenic present), false (tested negative with `status='absent'`), null (unknown/indeterminate/no findings). The ternary is smoke-verified on staging. Remaining: add del(17p) evidence, resolve contradictory sources, and define Likely pathogenic inclusion. Follow the [rollout and consumer contract](genomics_architecture.md#genomic-features-and-finding-terminology) for production cache reconciliation.
