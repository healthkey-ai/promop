...
| Final candidate smoke verification | [#1317](https://github.com/healthkey-ai/promop/issues/1317) | The authenticated smoke runner passes locally, including interrupted-write cleanup. Render staging passed all seven API smoke checks on candidate `0c655fa`, with web/worker revisions verified before and after; synthetic patient 7092 retains retired test history and no active findings. Repeat after the final candidate is deployed, and retain SHA, results and artifact accounting. |
