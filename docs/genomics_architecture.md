...
Baseline `dev` through `c6c7676`, including [#1249](https://github.com/healthkey-ai/promop/pull/1249), [#1259](https://github.com/healthkey-ai/promop/pull/1259), the [schema audit #1263](https://github.com/healthkey-ai/promop/pull/1263), [PALB2 naming #1281](https://github.com/healthkey-ai/promop/pull/1281), [project compatibility #1294](https://github.com/healthkey-ai/promop/pull/1294), [TP53 true/unknown #1297](https://github.com/healthkey-ai/promop/pull/1297), [TP53 cache reconciliation #1321](https://github.com/healthkey-ai/promop/pull/1321), [TP53 ternary fix #1360](https://github.com/healthkey-ai/promop/pull/1360), [concept display names #1355](https://github.com/healthkey-ai/promop/pull/1355), [TP53 disease matching fix #1357](https://github.com/healthkey-ai/promop/pull/1357), and the [CancerBot field mapping reconciliation #1284](https://github.com/healthkey-ai/promop/pull/1284). Updated 2026-09-16. Remaining requirements and deployment work are in [the genomics plan](genomics_plan.md).
...
For writer prerequisite verification, use `python manage.py audit_genomics_domains --include-writer-prerequisites`. In addition to the 31 component checks, this validates all 42 effective priority parent recipes using the writer's approval/source checks and Measurement-parent requirement. It requires active unmapped concept 0 and default actor concepts 32817 and 32865, and resolves the CDM event identity through the writer's own selector: the literal `measurement.measurement_id` code or the standard Athena concept with that exact name. Missing, rejected, incomplete or wrong-table parents and unavailable required concepts produce a nonzero exit. All failures are reported alongside component results; success is printed only when the requested checks pass.

This option does not modify recipes or facts. It reports parents using concept 0 without rejecting that supported source-only representation: an Observation-domain LOINC parent still belongs in Measurement for event linkage. The check does not certify clinical meanings, a full write transaction, schema/sequence/permission readiness, or additional type IDs supplied by service callers. The command without the option remains a component-domain audit, and earlier component-only deployment receipts do not establish these additional prerequisites.

After loading vocabulary, run the combined audit on that deployment. For each proposed repair, review the field in the field-mapping curation interface, set the installed standard concept and matching OMOP table, retain the source key and independent value/unit settings, and record the decision before approving it. Rerun the audit and retain its output with the vocabulary version. Curation applies to future writes; migrating existing facts between tables requires a separately reviewed data repair. Passing fixture tests does not certify a deployment vocabulary.
