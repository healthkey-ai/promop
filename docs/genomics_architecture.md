...
Detected-marker summaries use the effective present state, and refresh clears stale summaries after absence or indeterminate transitions. Derivation version 8 preserves the existing TP53 positive predicate and returns `true` only for a Pathogenic TP53 finding satisfying its legacy present-status/assessment checks; every other input yields `null` (unknown), never a derived `false`. Missing legacy status/assessment retains its existing positive interpretation. Absent, indeterminate, no-call, not-tested, unrelated and benign findings cannot establish a negative aggregate. The change does not add Likely pathogenic findings, del(17p), assay completeness or source-conflict rules. API and eligibility consumers must preserve null rather than coerce it to false. See the [TP53 rollout and consumer contract](tp53_aggregate_plan.md); broader clinical aggregation and external consumer acceptance remain open.
