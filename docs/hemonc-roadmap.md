...
| **Component concept_ids on the read model (SHIPPED, #231/#189)** | `PatientRecord.first_line_component_ids`, `second_line_component_ids`, `later_component_ids`, `therapy_component_ids` (`omop_core/models.py:1770-1785`), derived in `patient_record_service.py:558` (`_expand_component_ids`) at refresh | Union of regimen graph components + line DrugExposure concept_ids + `Maps to`/`Has ingredient` targets. **Mixed identifier levels, no type discriminator; compatibility updates map to OMOP then refresh this projection.** |
