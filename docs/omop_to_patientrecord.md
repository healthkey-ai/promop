...
| `tumor_size` | pending vocabulary registry decision | **Not enabled by #507.** Existing code uses LOINC 21889-1 for both tumour size and largest lymph-node size; a Measurement row cannot safely populate two distinct clinical meanings. Resolve the source-code collision with the Athena vocabulary before making this field read-only. |
