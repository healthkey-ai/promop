...
Observed gap. Explicit status and the empty-list contract work, but independent legacy assessment can contradict status. The absent-field guard covers only four older components, leaving newer variant-specific values outside validation. The list hides status and the dialog exposes two overlapping state controls. Units currently default to % despite the reference design's explicit-unit language. TP53 disruption still returns false when there is no evidence.

Work. Establish one effective-state rule using explicit status, with a source-preserving compatibility policy for legacy assessment. Reject contradictory new writes. Do not indiscriminately backfill legacy rows: preserve default-to-present compatibility while resolving existing explicit negative/no-call assessments without reclassifying them as detected findings. Obtain clinical/source agreement on not-tested, no-call and below-reporting-threshold results.

Review absent applicability across the full component set, particularly transcript DNA change and protein change type. Reject supplied inapplicable values; define and test inherited-value clearing on present-to-absent edits. Preserve a supplied negative source assertion and all applicable context. An empty named list must continue to mean unknown; neither placeholders nor other markers imply absence.

Expose clone fraction plus unit, transcript DNA change, coverage depth and amino-acid change type in the shared dialog. Show effective finding status in the list and make the empty-row/unknown distinction clear. Keep VAF and clone fraction separate. Require units in new source-import contracts; explicitly decide whether the interactive API keeps its % compatibility default or migrates to required units rather than changing existing caller behavior silently.

Review detected summaries and downstream trial matching for present, absent, indeterminate and unknown, including changes between states. Define an evidence-aware TP53 disruption projection; a gene-only negative must not be treated as absence of every related structural abnormality. Establish the clinical rule for combined TP53/del(17p) evidence before changing that aggregate.

Acceptance. Status round-trips on general and named routes; contradictory inputs and inapplicable absent components are covered; empty lists write no negative facts. UI tests exercise all added controls and state changes. Consumer tests show unknown/no-call are not converted to negatives, explicit absence is interpreted only within supported scope, and present-to-nonpositive transitions clear stale detected summaries.
