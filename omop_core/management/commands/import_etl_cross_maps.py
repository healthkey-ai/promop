...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--cpt-snomed-file', type=str, default=str(DEFAULT_CPT_SNOMED), help='Path to cpt_to_snomed_map.json')
        parser.add_argument('--snomed-rxnorm-file', type=str, default=str(DEFAULT_SNOMED_RXNORM), help='Path to snomed_to_rxnorm_map.json')
        parser.add_argument('--dry-run', action='store_true', help='Report counts without writing.')
        parser.add_argument('--limit', type=int, default=0, help='Max rows to process per file (0 = unlimited).')
        parser.add_argument('--skip-cpt', action='store_true', help='Skip CPT→SNOMED import.')
        parser.add_argument('--skip-rxnorm', action='store_true', help='Skip SNOMED→RxNorm import.')
    ...
    DOMAIN_TO_TABLE = {
        'Drug': 'drug_exposure', 'Procedure': 'procedure', 'Condition': 'condition',
    ...
    def _import_cpt_snomed(self, file_path, dry_run, limit):
        ...
        target_concepts = {c.concept_id: c for c in Concept.objects.filter(concept_id__in=target_ids)}
        source_ids = {int(e['cptConceptId']) for e in mapped_entries}
        source_concepts = {c.concept_id: c for c in Concept.objects.filter(concept_id__in=source_ids)}
        created = skipped = conflicts = missing_target = 0
        for entry in mapped_entries:
            ...
            if not target_concept:
                missing_target += 1
                if not dry_run:
                    logger.warning('CPT %s: target SNOMED concept_id %d not in DB', source_code, target_concept_id)
                continue
            if dry_run:
                created += 1
                continue
            ...
            obj, was_created = SourceCodeConceptMapping.objects.get_or_create(
                ...
                defaults={
                    ...
                    'destination_vocabulary_id': 'SNOMED', 'omop_table': omop_table,
            ...
            if was_created:
                ...
                if obj.target_concept_id and obj.target_concept_id != target_concept_id:
                    conflicts += 1
                    logger.warning('CPT %s: existing target_concept=%s differs from ETL target=%d', source_code, obj.target_concept_id, target_concept_id)
                else:
                    logger.debug('CPT %s: already mapped, skipping', source_code)
        verb = 'Would create' if dry_run else 'Created'
        self.stdout.write(self.style.SUCCESS(
            f'CPT→SNOMED: {verb} {created} rows. Skipped {skipped} existing, '
            f'{no_mapping} unmapped (snomedId=0), {missing_target} missing target concept, {conflicts} conflicts.'
        ))
    ...
    def _import_snomed_rxnorm(self, file_path, dry_run, limit):
        ...
        rxnorm_concepts = {c.concept_code: c for c in Concept.objects.filter(vocabulary_id='RxNorm', concept_code__in=rxnorm_codes)}
        snomed_codes = set(data.keys())
        snomed_concepts = {c.concept_code: c for c in Concept.objects.filter(vocabulary_id='SNOMED', concept_code__in=snomed_codes)}
        created = skipped = conflicts = missing_target = 0
        ...
        for snomed_code, rxnorm_code in entries:
            ...
            if not rxnorm_concept:
                missing_target += 1
                if not dry_run:
                    logger.warning('SNOMED %s: RxNorm code %s not found in DB', snomed_code, rxnorm_code)
                continue
            if dry_run:
                created += 1
                continue
            ...
            obj, was_created = SourceCodeConceptMapping.objects.get_or_create(
                ...
                defaults={
                    ...
                    'source_concept': snomed_concept, 'target_concept': rxnorm_concept,
            ...
            if was_created:
                ...
                if obj.target_concept_id and obj.target_concept_id != rxnorm_concept.concept_id:
                    conflicts += 1
                    logger.warning('SNOMED %s: existing target_concept=%s differs from ETL RxNorm concept=%d', snomed_code, obj.target_concept_id, rxnorm_concept.concept_id)
                else:
                    logger.debug('SNOMED %s: already mapped, skipping', snomed_code)
        verb = 'Would create' if dry_run else 'Created'
        self.stdout.write(self.style.SUCCESS(
            f'SNOMED→RxNorm: {verb} {created} rows. Skipped {skipped} existing, '
