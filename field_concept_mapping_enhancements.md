...
The latest published `vocabulary_release` row is ID 11, Athena version
...
| Genetic genes/variants/origins/interpretations | Inventory every dependent option and gene/variant relationship. Resolve exact variants with gene, transcript/reference assembly and method context where applicable; retain unmapped source identifiers. Reuse existing mutation models and structured `genetic_mutations`. |
